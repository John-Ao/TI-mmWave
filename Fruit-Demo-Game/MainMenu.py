...
pygame.display.set_caption("Fruit Demo Game")
...
class Button():
    ...
    def draw(self):
        ...
        if self.x <= pos[0] <= self.x + self.width and self.y <= pos[1] <= self.y + self.height:
            pygame.draw.rect(gameDisplay,(0,135,193),(self.x,self.y,self.width,self.height),0)
        else:
            pygame.draw.rect(gameDisplay,(0,86,123),(self.x,self.y,self.width,self.height),0)
        pygame.draw.rect(gameDisplay,(100,100,100),(self.x,self.y,self.width,self.height),5)

        if self.text != "Change Color":
            text_surface, rect = font_50.render(str(self.text), (255,255,255))
            gameDisplay.blit(text_surface, (self.x + int(self.width/2)+30 - int(SizeCheck.size(str(self.text))[0]), self.y + int(self.height/2) - 20))
        else:
            text_surface, rect = font_35.render(str("Change"), (255,255,255))
            gameDisplay.blit(text_surface,(565,615))
            text_surface, rect = font_35.render(str("Color"), (255,255,255))
...
def HomeScreen(score=0):
    ...
    while game_run == True:
        ...
        pos = pygame.mouse.get_pos()
        if screen == "Main":
            text_surface, rect = font_100.render(("Fruit Demo Game"), (255,255,255))
            gameDisplay.blit(text_surface, (100, 100))
